...
9.В 0-й фазе должен быть еще лэйбл, показывающий сколько еще можно разместить
11.В Game.play_ сделать возможность нескольким игрокам играть
12.Game.play(...) не должен вызываться нажатием на кнопку
!!!15.реализовать ИИ перемещение
16.В Game.move(...) армия при захвате не должна делиться пополам. Использовать ComboBox
17.Удалять игроков из списка при их поражении
18.Проверять, не обращаюсь ли я к "нулевому" элементу массива, когда ищу в его массиве сектор
19.В ComputerBehavior.play1(...) компьютер не должен нападать по всем территориям по всем направлениям
?22е невилирует? 20.Сделать первый ход без нападений или разбросать армии компа по территориям заранее
?22е невилирует? 21.Установить лимит отрядов на одной ячейке
!!!22. Компьютер непонятно когда останавливает атаку, нужно установить лимит
!!!23. Перестает работать при смерти одного из игроков
!!!24. Реализовать конец игры
 */
    ...
    public void draw(Stage primaryStage){
        ...
        for(yNum = 0;yNum < 10; yNum++){     //строка
            for(xNum = 0;xNum < 10;xNum++){  //столбец
                ...
                switch (cell.owner){
                    case 1 :
                        pl1.terr[xNum + yNum*10] = cell;
                        break;
                    case 2 :
                        pl2.terr[xNum + yNum*10] = cell;
                        break;
                    case 3 :
                        pl3.terr[xNum + yNum*10] = cell;
                        break;
                    case 4 :
                        pl4.terr[xNum + yNum*10] = cell;
                        break;
                }
            }
        }
